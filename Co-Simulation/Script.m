...
%% -------------- SimuLink Model ---------------------   
    Jload = 0.01;  %Jg = 0;Jload = 0; Bg =0; %i = 63;
 ...
 %% -------------- Open-Loop Xfer Function --------------  
    ...
    bestKp = RecursiveSearch(1e-5,10, 1, margin(OLXferD)/200, time, Foward,FBCK, 1,10, 60, 1)
...
subplot(2,3,2);
plot (t2*1e3,torque*i,'color', 'blue','LineWidth',2.25,'LineStyle','-','DisplayName','Torque \tau_M');
...
if(config == 1)     
    if(FirstOrderFlag==0) 
        ...
        if(LowestTime>=PeakTime + RiseTime + SettleTime  && SettleTime < 1.5 &&  Overshoot < overshootPercent ) 
